...
def new_destination(user, address, phone_number, description):
    try:
        sql = """INSERT INTO destinations (user_id, address, visible)
                 VALUES (:user, :address, 1) RETURNING id"""
        destination_id = db.session.execute(sql, {"user":user, "address":address}).fetchone()[0]

        sql = """INSERT INTO info (destination_id, phone_number, description)
                 VALUES (:destination_id, :phone_number, :description)"""
...
def add_review(user_id, destination_id, stars, comment):
    sql = """INSERT INTO reviews (destination_id, user_id, stars, comment)
             VALUES (:destination_id, :user_id, :stars, :comment)"""
    db.session.execute(sql, {"destination_id":destination_id, "user_id":user_id,
                             "stars":stars, "comment":comment})
    db.session.commit()

def get_reviews(destination_id):
    sql = """SELECT u.id, u.username, r.stars, r.comment FROM reviews r, users u, destinations d
             WHERE r.user_id=u.id AND r.destination_id=:destination_id AND d.id=r.destination_id
             AND d.visible=1 ORDER BY r.id"""
    return db.session.execute(sql, {"destination_id": destination_id}).fetchall()

def get_destination_stars(destination_id):
    sql = "SELECT AVG(stars) FROM destinations WHERE destination_id=:destination_id AND visible=1"
